...
class BFS():
    ...
    def __init__(self, network=None, target=None,  max_pathways=50000, max_pathway_length=5, min_weight=1, use_random=False,
        ...
        """
        self.allow_term_spontanuous_rxns = False
        ...
        if self.network == None:
            self.target = retrobiocat_web.retro.network_pathway.rdkit_utils.rdkit_smile(target, warning=True)
    ...
    def _pick_choice(self, context):
        ...
        def pick_best(choices, scores):
            sorted_options = sort_by_score(choices, scores, reverse=False)
    ...
    def _check_pathway_has_end(self, nodes):
        ...
        end_nodes = retrobiocat_web.retro.network_pathway.graph_functions.get_nodes_with_no_successors(pathway_subgraph)
        if len(end_nodes) == 0:
            return False
        return True
    ...
    def get_pathways(self):
        ...
        for list_nodes in self.pathways:
            ...
            if not pathway.does_pathway_end_in_spontaneous_chemical_step() or self.allow_term_spontanuous_rxns:
                if self.allow_longer_pathways:

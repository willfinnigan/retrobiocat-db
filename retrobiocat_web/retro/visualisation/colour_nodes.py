...
def colour_target_node(nodes, graph):
    for node_dict in nodes:
        node = node_dict['id']
        if 'target' in graph.nodes[node]['attributes']:
            nodes = mark_node(nodes, node, 'orange')
    return nodes
    ...
    for i, node_dict in enumerate(nodes):
        ...
        if graph.nodes[node]['attributes']['node_type'] == 'reaction':
            if graph.nodes[node]['attributes']['reaction_type'] == 'retrobiocat':
                ...
                if 'specificity_scores' in graph.nodes[node]['attributes']['metadata']:
                    activity = float(graph.nodes[node]['attributes']['metadata']['specificity_scores'][enz])

...
def add_enzyme_info(nodes, graph, img_size=(150,150)):
    ...
    for i, node_dict in enumerate(nodes):
        ...
        if graph.nodes[node]['attributes']['node_type'] == 'reaction':
            if graph.nodes[node]['attributes']['reaction_type'] == 'retrobiocat':
                enz = graph.nodes[node]['attributes']['metadata']['selected_enzyme']
                if 'enzyme_info' in graph.nodes[node]['attributes']['metadata']:
                    info = graph.nodes[node]['attributes']['metadata']['enzyme_info'][enz]
                    score = graph.nodes[node]['attributes']['metadata']['specificity_scores'][enz]

...
class BlastParser(object):
    ...
    @staticmethod
    def _get_sequence(identifier, retry_num=3, retry_wait=60):
        for i in range(retry_num):
            try:
                ...
                print(f'Issue with uniprot connection, wait {retry_wait} seconds before retry {i+1} of {retry_num}')
...
def parse_blast_results(enzyme_name, output):
    ...
    if output is not None:
        if BlastParser().parse(output, seq):
            seq.blast = datetime.datetime.now()
            seq.save()
            print(f'Finished blast of sequence {seq.enzyme_name}')

    job_id = f"update_blast_status_for_{seq.enzyme_type}_due_to_{seq.enzyme_name}_blast_parse_finish"
    current_app.task_queue.enqueue(update_blast_status, seq.enzyme_type, job_id=job_id)

def update_blast_status(enzyme_type):
    ...
    if all_complete == True:
        if enz_type_obj.bioinformatics_status != 'Complete':
            ...
            enz_type_obj.save()
            job_id = f"blast_complete_now_get_uniref_info_for_{enz_type_obj.enzyme_type}_enzymes"
            current_app.db_queue.enqueue(task_get_uniref_info, enzyme_type, False, job_id=job_id)
...
def set_blast_jobs(enzyme_type):
    ...
    current_app.blast_queue.enqueue(update_blast_status, enzyme_type)
...
if __name__ == '__main__':
    ...
    br = BlastRunner()
    br.db = 'uniref100'   #UNIPROTKB_SWISSPROT ?
    br.alignments = 50
    xml = br.run(protein_seq)

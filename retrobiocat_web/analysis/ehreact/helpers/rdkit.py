...
import re
import itertools
from retrobiocat_web.analysis.ehreact.helpers.transition_state import process_an_example

def get_mcs_mol(mols):
    if len(mols) == 0:
...
def set_seed_atoms_no_implicit(mol_seed):
    for atom in mol_seed.GetAtoms():
        # Set everything to "NoImplicit" in templates, we never want to add Hs without explicitly specifying
        atom.SetNoImplicit(True)

def get_matches(mol_seed, smiles, smiles_dict):
    smi_matches = []
    for smi in smiles:
        mol = smiles_dict[smi]["mol_no_stereo"]
        matches = mol.GetSubstructMatches(mol_seed)
        for match in matches:
            if do_charges_fit(mol, mol_seed, match):
                smi_matches.append(smi)
                break
    return smi_matches
...
def preprocess_seeds(seed_list, smiles, smiles_dict):
    ...
    for seed in seeds:
        ...
        # set seed molecule
        mol_seed = Chem.MolFromSmiles(seed, sanitize=False)
        set_seed_atoms_no_implicit(mol_seed)

        # get matches
        matches = get_matches(mol_seed, smiles, smiles_dict)
        ...
        num_smiles_seed.append(len(matches))

...
def load_single_activity_data():
    ...
    seq = Sequence.objects(enzyme_name=activity.enzyme_name).first()
    other_names_str = ''
    if seq is not None:
        if len(seq.other_names) != 0:
            other_names_str = '(' + ', '.join(seq.other_names) + ')'
    ...
    if activity.temperature is None:
        ...
        temperature = f'{activity.temperature} <sup>o</sup>C'

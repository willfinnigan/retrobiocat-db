...
def df_to_json_tabulate(df, img_path='static/specificity_images/'):
    ...
    for index, row in df.iterrows():
        substrate_1 = retrobiocat_web.retro.network_pathway.rdkit_utils.rdkit_smile(str(row['Substrate 1 SMILES']))
        substrate_2 = retrobiocat_web.retro.network_pathway.rdkit_utils.rdkit_smile(str(row['Substrate 2 SMILES']))
        product_1 = retrobiocat_web.retro.network_pathway.rdkit_utils.rdkit_smile(str(row['Product 1 SMILES']))
